...
export default function Battle() {
  ...
  const attackPreview = useMemo(() => {
    ...
    const defenseMult = facingDefenseMultiplier(front);
    ...
    if (isSkillMode) {
      ...
      const base = Math.max(1, Math.floor(attacker.attack * skill.dmgMultiplier - target.defense * (1 / penFactor)));
    ...
    // Basic attack
    const base = Math.max(1, Math.floor((attacker.attack * 1.0 - target.defense * defenseMult) + 0));
    const lo = Math.max(1, base - 2);
    const hi = base + 3;
